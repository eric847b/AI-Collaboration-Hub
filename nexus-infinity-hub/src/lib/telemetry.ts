 ...
 * (window.onerror), unhandled promise rejections, and `console.error` calls
 * (React render errors reach this via ErrorBoundary). Entries live in a
 * bounded in-memory ring buffer, persist to localStorage (best-effort) and,
 * ONLY when an endpoint is configured via `installTelemetry({ endpoint })`
 * or `window.TELEMETRY_ENDPOINT`, batch-flush as JSON through
 * sendBeacon/fetch-keepalive. Without an endpoint the hook stays fully
 * local: it never touches the network.
 *
 * Privacy: entries contain kind/message/stack/path only, length-capped; no
 * cookies, form values, or other page content are captured.
 */
...
/** Window `error` listener body — exported for unit tests. */
...
/** Window `unhandledrejection` listener body — exported for unit tests. */
