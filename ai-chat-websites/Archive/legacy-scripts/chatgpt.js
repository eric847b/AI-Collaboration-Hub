...
const chatgpt = {
  ...
  actAs(persona) {
    ...
    const promptsUrl =
      "https://raw.githubusercontent.com/KudoAI/chat-prompts/main/dist/personas.min.json";
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status !== 200)
          return reject("🤖 chatgpt.js >> Request failed. Cannot retrieve prompts data.");
        ...
        chatgpt.isIdle().then(() => {
          console.info("Persona activated!");
        });
        return resolve();
      };
    });
  ...
  alert(title, msg, btns, checkbox, width) {
    ...
    if (!modalStyle || parseInt(modalStyle.getAttribute("last-updated"), 10) < thisUpdated) {
    ...
    chatgpt.renderHTML(modalMessage);
    ...
    if (alertQueue.length === 1) {
      ...
      setTimeout(() => {
        // delay non-0 opacity's for transition fx
        modalContainer.style.backgroundColor = `rgba(67, 70, 72, ${scheme === "dark" ? 0.62 : 0.1})`;
      ...
      if (dismissKeys.includes(event.key) || dismissKeyCodes.includes(event.keyCode)) {
        for (const alertId of alertQueue) {
          ...
          if (alert && alert.style.display !== "none") {
            ...
            {
              dismissAlert();
            } // dismiss alert & do nothing
            else if (
              [" ", "Spacebar", "Enter", "Return"].includes(event.key) ||
      ...
      setTimeout(() => {
        ...
        if (alertQueue.length > 0) {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              nextAlert.classList.add("animated");
            }, 100);
          }, 500);
        }
      }, 50);
  ...
  autoRefresh: {
    ...
    deactivate() {
      if (this.isActive) {
        ...
        this.isActive = null;
        console.log(
          `↻ ChatGPT >> [${chatgpt.autoRefresh.nowTimeStamp()}] Auto refresh de-activated`
        );
      } else {
        console.log(
          `↻ ChatGPT >> [${chatgpt.autoRefresh.nowTimeStamp()}] Auto refresh already inactive!`
        );
      }
    },
  ...
  browser: {
    ...
    isChromium() {
      return Boolean(JSON.stringify(navigator.userAgentData?.brands)?.includes("Chromium"));
    },
    isChrome() {
      return Boolean(JSON.stringify(navigator.userAgentData?.brands)?.includes("Chrome"));
    },
    isEdge() {
      return Boolean(JSON.stringify(navigator.userAgentData?.brands)?.includes("Edge"));
    },
    isBrave() {
      return Boolean(JSON.stringify(navigator.userAgentData?.brands)?.includes("Brave"));
    },
    isFirefox() {
    ...
    return new Promise((resolve, reject) => {
      chatgpt
        ...
        .then((token) => {
          ...
          xhr.onload = () => {
            if (xhr.status !== 200)
              return reject("🤖 chatgpt.js >> Request failed. Cannot clear chats.");
  ...
  code: {
    ...
    extract(msg) {
      // extract pure code from response (targets last block)
    ...
    async isIdle(timeout = null) {
      const obsConfig = { childList: true, subtree: true },
        selectors = {
          msgDiv: "div[data-message-author-role]",
      ...
      const isIdlePromise = (async () => {
        await new Promise((resolve) => {
          ...
          if (document.querySelector(selectors.msgDiv)) resolve();
            ...
            new MutationObserver((_, obs) => {
              if (document.querySelector(selectors.msgDiv)) {
                obs.disconnect();
                resolve();
              }
        ...
        await new Promise((resolve) => {
          // when reply starts generating
          new MutationObserver((_, obs) => {
            if (chatgpt.getStopBtn()) {
              obs.disconnect();
              resolve();
            }
        ...
        await new Promise((resolve) => {
          // when code starts generating
          new MutationObserver((_, obs) => {
            if (lastReplyDiv?.querySelector("pre")) {
              obs.disconnect();
              resolve();
            }
          }).observe(document.body, obsConfig);
        ...
        return new Promise((resolve) => {
          // when code stops generating
          new MutationObserver((_, obs) => {
            if (
              lastReplyDiv?.querySelector("pre")?.nextElementSibling || // code block not last child of reply div
    ...
    async write(prompt, outputLang) {
      ...
      if (!outputLang)
        return console.error("outputLang (2nd) argument not supplied. Pass a language!");
      for (let i = 0; i < arguments.length; i++) {
        if (typeof arguments[i] !== "string") {
          return console.error(`Argument ${i + 1} must be a string.`);
        }
  ...
  async exportChat(chatToGet, format) {
    ...
    // Init args
    chatToGet = !chatToGet
      ? "active" // default to 'active' if unpassed
    ...
    if (/te?xt/.test(format)) {
      // generate plain transcript + filename for TXT export
      ...
      if (chatToGet == "active" && /\/\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$/.test(window.location.href)) {
        const chatDivs = document.querySelectorAll(
          "main > div > div > div > div > div > div[class*=group]"
        );
        if (chatDivs.length === 0) return console.error("Chat is empty!");
        const msgs = [];
        let isUserMsg = true;
        chatDivs.forEach((div) => {
          const sender = isUserMsg ? "USER" : "CHATGPT";
          isUserMsg = !isUserMsg;
          const msg = Array.from(div.childNodes)
      ...
      // generate rich transcript + filename for HTML/MD/PDF export
    ...
    if (format == "pdf") {
      // convert SVGs + launch PDF printer
      ...
      // Launch PDF printer
      const transcriptPopup = window.open(
        "",
        "",
        "toolbar=0, location=0, menubar=0, height=600, width=800"
      );
      transcriptPopup.document.write(transcript);
      setTimeout(() => {
        transcriptPopup.print({ toPDF: true });
      }, 100);
    } else {
      // auto-save to file

      if (format == "md") {
        // remove extraneous HTML + fix file extension
        const mdMatch = /<.*(?:<h1(.|\n)*?href=".*?continue[^"]*".*?\/a>.*?)<[^/]/.exec(
          transcript
        )[1];
        transcript = mdMatch || transcript;
  ...
  footer: {
    get() {
      return document.querySelector("main form")?.parentNode.parentNode.nextElementSibling;
    },
  ...
  get(targetType, targetName = "") {
    ...
    if (!cjsTargetTypes.includes(targetType.toLowerCase())) {
      throw new Error(
        `Invalid targetType: ${targetType}. Valid values are: ${JSON.stringify(cjsTargetTypes)}`
      );
    }
    ...
    const targetFuncName = Object.keys(this).find(
  ...
  getAccessToken() {
    return new Promise((resolve, reject) => {
      if (
        Object.keys(chatgpt.openAIaccessToken).length > 0 && // populated
      ...
      xhr.onload = () => {
        if (xhr.status !== 200)
          return reject("🤖 chatgpt.js >> Request failed. Cannot retrieve access token.");
        console.info(
          `Token expiration: ${new Date(JSON.parse(xhr.responseText).expires).toLocaleString().replace(",", " at")}`
        );
  ...
  getAccountDetails(...details) {
    ...
    details = !arguments[0]
      ? validDetails // no details passed, populate w/ all valid ones
      : Array.isArray(arguments[0])
        ? arguments[0] // details array passed, do nothing
  ...
  getChatData(chatToGet = 1, detailsToGet = "all", sender = "all", msgToGet = "all") {
    ...
    chatToGet = !chatToGet
      ? "active" // if '' passed, set to active
      : Number.isInteger(chatToGet) || /^\d+$/.test(chatToGet) // else if string/int num passed
        ? parseInt(chatToGet, 10) === 0
          ? 0
          : parseInt(chatToGet, 10) - 1 // ...offset -1 or keep as 0
        : chatToGet; // else preserve non-num string as 'active', 'latest' or title/id of chat to get
    detailsToGet = ["all", ""].includes(detailsToGet) // if '' or 'all' passed
      ...
      : Array.isArray(detailsToGet)
        ? detailsToGet
        : [detailsToGet]; // else convert to array if needed
    sender = !sender
      ? "all" // if '' or unpassed, set to 'all'
      : validSenders.includes(sender)
        ? sender
        : "invalid"; // else set to validSenders or 'invalid'
    msgToGet =
      Number.isInteger(msgToGet) || /^\d+$/.test(msgToGet) // if string/int num passed
        ? parseInt(msgToGet, 10) === 0
          ? 0
          : parseInt(msgToGet, 10) - 1 // ...offset -1 or keep as 0
        : ["all", "latest"].includes(msgToGet.toLowerCase()) // else if 'all' or 'latest' passed
          ? msgToGet.toLowerCase() // ...preserve it
          : !msgToGet
            ? "all" // else if '', set to 'all'
    ...
    const getChatDetails = (token, detailsToGet) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          ...
          if (
            Number.isInteger(chatToGet) ||
            chatToGet == "latest" ||
            (chatToGet == "active" &&
              !new RegExp(`\/${re_chatID.source}$`).test(window.location.href))
          ) {
            chatToGet = Number.isInteger(chatToGet) ? chatToGet : 0; // preserve index, otherwise get latest
            if (chatToGet > data.length) {
              // reject if index out-of-bounds
              return reject(
                `🤖 chatgpt.js >> Chat with index ${
                  chatToGet + 1
          ...
          const chatIdentifier = // determine to check by ID or title
            chatToGet == "active" || new RegExp(`^${re_chatID.source}$`).test(chatToGet)
              ? "id"
              : "title";
          ...
          for (idx = 0; idx < data.length; idx++) {
            // search for id/title to set chatFound flag
      ...
      return new Promise((resolve, reject) => {
        ...
        getChatDetails(token, ["id"]).then((chat) => {
          ...
          xhr.onload = () => {
            ...
            {
              return reject(
                `🤖 chatgpt.js >> Message/response with index ${
                  msgToGet + 1
            ...
            for (const userMessage of userMessages) {
              ...
              for (const key in data) {
                if (
                  ...
                  data[key].message.author.role == "assistant" &&
            ...
            } else if (sender == "chatgpt") // Fill [msgsToReturn] with ChatGPT responses
            {
              for (const chatGPTMessage of chatGPTMessages) {
                msgsToReturn.push(
                  msgToGet == "latest"
                    ? chatGPTMessages[chatGPTMessages.length - 1]
                    : chatGPTMessage
                );
              }
            } else {
              // Fill [msgsToReturn] with objects of user messages and chatgpt response(s)
              let i = 0;
              for (const message in userMessages) {
                msgsToReturn.push({
                  ...
                  chatgpt:
                    msgToGet == "latest"
                      ? chatGPTMessages[i][chatGPTMessages[i].length - 1]
                      : chatGPTMessages[i],
                });
                i++;
              }
            ...
            return resolve(
              msgToGet == "all"
                ? msgsToReturn // if 'all' passed, return array
                : msgToGet == "latest"
                  ? msgsToReturn[msgsToReturn.length - 1] // else if 'latest' passed, return latest
                  : msgsToReturn[msgToGet]
            ); // else return element of array
          };
          xhr.send();
        });
      });
    ...
    return new Promise((resolve) =>
      chatgpt.getAccessToken().then((token) => {
        return resolve(
          detailsToGet.includes("msg") ? getChatMsgs(token) : getChatDetails(token, detailsToGet)
        );
      })
    );
  },
  ...
  getContinueButton() {
    return document.querySelector('button:has([d^="M4.47189"])');
  },
  getFooterDiv() {
    return chatgpt.footer.get();
  },
  getHeaderDiv() {
    return chatgpt.header.get();
  },
  getLastPrompt() {
    return chatgpt.getChatData("active", "msg", "user", "latest");
  },
  getLastResponse() {
    return chatgpt.getChatData("active", "msg", "chatgpt", "latest");
  },
  ...
  getNewChatLink() {
    return document.querySelector('nav a[href="/"]');
  },
  getRegenerateButton() {
    return document.querySelector('button:has([d^="M3.06957"])');
  },
  ...
  getResponseFromAPI(chatToGet, responseToGet) {
    return chatgpt.response.getFromAPI(chatToGet, responseToGet);
  },
  getResponseFromDOM(pos) {
    return chatgpt.response.getFromDOM(pos);
  },
  getScrollToBottomButton() {
    return document.querySelector('button:has([d^="M12 21C11.7348"])');
  },
  getSendButton() {
  ...
  header: {
    get() {
      return document.querySelector("main .sticky");
    },
    hide() {
  ...
  history: {
    async isLoaded(timeout = null) {
      ...
      return await (timeoutPromise
        ? Promise.race([isLoadedPromise, timeoutPromise])
        : isLoadedPromise);
    },
  },
    ...
    sendRequest(method, token, body) {
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          ...
          if (xhr.status === 422) {
            return reject(
              "🤖 chatgpt.js >> Character limit exceeded. Custom instructions can have a maximum length of 1500 characters."
            );
            ...
            return reject(
              "🤖 chatgpt.js >> Request failed. Cannot contact custom instructions endpoint."
            );
          }
  ...
  isFullScreen() {
    return chatgpt.browser.isFullScreen();
  },
    ...
    const isIdlePromise = (async () => {
      await new Promise((resolve) => {
        ...
        if (document.querySelector(msgDivSelector)) resolve();
          ...
          new MutationObserver((_, obs) => {
            if (document.querySelector(msgDivSelector)) {
              obs.disconnect();
              resolve();
            }
          }).observe(document.body, obsConfig);
  ...
  isLightMode() {
    return document.documentElement.classList.toString().includes("light");
  },
  ...
  menu: {
    ...
    append(element, attrs = {}) {
      ...
      {
        return console.error(
          `🤖 chatgpt.js >> Invalid element! Valid elements are [${validElements}]`
        );
      }

      const newElement = document.createElement(
        element == "dropdown" ? "select" : element == "button" ? "a" : element
      );
      newElement.id = Math.floor(chatgpt.randomFloat() * 1000000) + Date.now(); // add random id to the element

      if (element == "button") {
        newElement.textContent =
          attrs?.label && typeof attrs.label === "string" ? attrs.label : "chatgpt.js button";

        const icon = document.createElement("img");
        icon.src =
          attrs?.icon && typeof attrs.icon === "string" // can also be base64 encoded image string
        ...
        newElement.onclick =
          attrs?.onclick && typeof attrs.onclick === "function" ? attrs.onclick : function () {};
      } else if (element == "dropdown") {
        if (
          !attrs?.items || // there no are options to add
        ...
        if (
          !attrs.items.every((el) => typeof el === "object")
        ) // the entries of the array are not objects
        {
      ...
      const addElementsToMenu = () => {
        ...
        chatgpt.menu.elements.forEach((element) => {
          ...
          if (!headlessNav.contains(element)) {
            try {
              headlessNav.insertBefore(element, headlessNav.firstChild);
            } catch (err) {
              console.error(err);
            }
          }
      ...
      if (!this.addedEvent) {
        // to prevent adding more than one event
        menuBtn?.addEventListener("click", () => {
          setTimeout(addElementsToMenu, 25);
        });
    ...
    close() {
      try {
        document.querySelector('nav [id*="menu-button"][aria-expanded="true"]').click();
      } catch (err) {
    ...
    open() {
      try {
        document.querySelector('nav [id*="menu-button"][aria-expanded="false"]').click();
      } catch (err) {
        console.error(err.message);
      }
    },
  ...
  notify(msg, position, notifDuration, shadow) {
    ...
    closeBtn.classList.add("notif-close-btn", "no-mobile-tap-outline");
    const closeSVG = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    ...
    if (!notifStyle || parseInt(notifStyle.getAttribute("last-updated"), 10) < thisUpdated) {
      ...
      notifStyle.innerText = // update prev/new style contents
        ...
        `-webkit-user-select: none ; -moz-user-select: none ; -ms-user-select: none ; user-select: none ;` +
        `transform: translateX(${!notificationDiv.isRight ? "-" : ""}35px) ;${
          // init off-screen for transition fx
          shadow
            ? `box-shadow: -8px 13px 25px 0 ${/\b(shadow|on)\b/gi.test(shadow) ? "gray" : shadow}`
            : ""
        }}` +
      ...
      try {
        ...
        for (const divId of thisQuadrantQueue.slice(0, -1)) {
          ...
          const oldDiv = document.getElementById(divId),
            ...
            vOffset =
              +/\d+/.exec(oldDiv.style[offsetProp])[0] + 5 + oldDiv.getBoundingClientRect().height;
  ...
  printAllFunctions() {
    ...
    Object.keys(colors).forEach((element) => {
      // populate dark scheme colors if missing
      colors[element][1] =
        colors[element][1] ||
        `#${(Number(`0x1${colors[element][0].replace(/^#/, "")}`) ^ 0xffffff)
          .toString(16)
          .substring(1)
          .toUpperCase()}`; // convert to hex
    });
    ...
    for (const prop in this) {
      if (typeof this[prop] === "function") {
        const chatgptIsParent = !Object.keys(this).find((obj) =>
            Object.keys(this[obj]).includes(this[prop].name)
          ),
    ...
    for (const functionName of functionNames) {
      ...
      console.log(
        `%c>> %c${isChatGptObjParent ? "" : `${functionName[0]}.%c`}${
          functionName[1]
  ...
  refactor() {
    chatgpt.code.refactor();
  },
  regenerate() {
    chatgpt.response.regenerate();
  },
  ...
  renderHTML(node) {
    ...
    if (!chatgpt.renderHTML.preWrapSet) {
      ...
      setTimeout(() => {
        chatgpt.renderHTML.preWrapSet = false;
      }, 100);
    }
  ...
  async resend() {
    chatgpt.send(await chatgpt.getChatData("latest", "msg", "user", "latest"));
  },

  response: {
    continue() {
      try {
        chatgpt.getContinueBtn().click();
      } catch (err) {
        console.error(err.message);
      }
    },
    ...
    getFromAPI(chatToGet, responseToGet) {
      ...
      chatToGet = chatToGet || "latest";
      responseToGet = responseToGet || "latest";
      ...
      if (/last|final/.test(strPos)) // get last response
      {
        ...
        const nthOfResponse =
          ...
          Number.isInteger(pos)
            ...
            : /^\d+/.test(strPos)
              ? /^\d+/.exec(strPos)[0] // extract first digits for strings w/ them
    ...
    regenerate() {
      try {
        chatgpt.getRegenerateBtn().click();
      } catch (err) {
        console.error(err.message);
      }
    },
  ...
  send(msg, method = "") {
    ...
    setTimeout(function delaySend() {
      ...
      if (!sendBtn?.hasAttribute("disabled")) {
        // send msg
        method.toLowerCase() == "click" || chatgpt.browser.isMobile()
          ? sendBtn.click()
  ...
  settings: {
    scheme: {
      ...
      set(value) {
        ...
        if (value == "system")
          schemeToSet = window.matchMedia("(prefers-color-scheme: dark)").matches
            ? "dark"
            : "light";
  ...
  setScheme(value) {
    chatgpt.settings.scheme.set(value);
  },
  ...
  sidebar: {
    ...
    append(element, attrs = {}) {
      ...
      if (element == "dropdown")
        newElement.style.backgroundColor = "var(--gray-900, rgb(32, 33, 35))";
    ...
    exists() {
      return Boolean(chatgpt.getNewChatLink());
    },
    hide() {
  ...
  async suggest(ideaType, details) {
    if (!ideaType) {
      return console.error(
        "ideaType (1st argument) not supplied" + "(e.g. 'gifts', 'names', 'recipes', etc.)"
      );
    }
    for (let i = 0; i < arguments.length; i++) {
      if (typeof arguments[i] !== "string") {
  ...
  async translate(text, outputLang) {
    ...
    chatgpt.send(
      `Translate the following text to ${outputLang}. Only reply with the translation.\n\n${text}`
    );
  ...
  uuidv4() {
    ...
    const uuid = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (c) => {
      const r = // generate random nibble
        ((d + (window.crypto.getRandomValues(new Uint32Array(1))[0] / (Math.pow(2, 32) - 1)) * 16) %
          16) |
        0;
...
const cjsBtnActions = ["click", "get"],
  cjsTargetTypes = ["button", "link", "div", "response"];
for (const btnAction of cjsBtnActions) {
  chatgpt[`${btnAction}Button`] = function handleButton(buttonIdentifier) {
    const button = /^[.#]/.test(buttonIdentifier)
      ? document.querySelector(buttonIdentifier)
...
const cjsFuncAliases = [
  ...
  [
    ...
    "deactivateAutoRefresher",
    "deactivateRefresher",
    "deactivateSessionRefresher",
  ],
...
const camelCaser = (words) => {
  return words
    .map((word, index) =>
      index === 0 || word == "s" ? word : word.charAt(0).toUpperCase() + word.slice(1)
    )
    .join("");
};
for (const prop in chatgpt) {
      ...
      if (typeof chatgpt[funcName] === "function") {
        ...
        for (const funcWord of funcWords) {
          const synonymValues = [].concat(
            ...cjsFuncSynonyms // flatten into single array w/ word's cjsFuncSynonyms
          ...
          for (const synonym of synonymValues) {
            ...
            const newFuncName = camelCaser(
              funcWords.map((word) => (word == funcWord ? synonym : word))
            );
...
const consolePrefix = "🤖 chatgpt.js >> ",
...
} catch (err) {} // for Greasemonkey
try {
  module.exports = chatgpt;
} catch (err) {} // for CommonJS
