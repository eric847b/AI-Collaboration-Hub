...
- [x] OPS Dashboard regenerated 2026-09-15 (`docs/metrics/OPS-DASHBOARD.md`) — tooling inventory now lists `extension-check.mjs` + `verify-tools.mjs` (was 13 files, now 15); `.vscode/tasks.json` local tasks for both added (gitignored via `.vscode/`, local-only enhancement, not committed)
