...
def skill_triage_issue(title, body):
    ...
    labels = []
    if any(k in text for k in ["bug", "error", "fail", "crash"]):
        labels.append("bug")
    if any(k in text for k in ["feat", "enhancement", "add "]):
        labels.append("enhancement")
    if any(k in text for k in ["doc", "readme"]):
        labels.append("documentation")
    if any(k in text for k in ["security", "vuln", "cve"]):
        labels.append("security")
    return {
        "labels": labels or ["triage"],
    ...
    try:
        ...
        if r.ok:
            for issue in r.json():
                ...
                if handled:
                    if dry_run:
                        closed.append({"number": issue["number"], "action": "would-close"})
                    else:
