...
Handles CI failure classes:
  ...
  - Python syntax errors (py_compile / ast.parse)
...
# Known-good action majors (bump when past EOL / deprecated)
GHA_KNOWN_GOOD = {
    "actions/checkout": "v4",
...
# Minimal pins for common Python packages when requirements.txt is missing
PYTHON_CORE_PINS = {
...
def _python_project_dirs(root: str = ".") -> List[str]:
    """Find directories that look like Python packages (have .py or pyproject/setup)."""
    dirs = set()
    for path in _walk_files(root, (".py",)):
        d = os.path.dirname(path) or "."
        # Prefer package root: nearest dir with pyproject.toml / setup.py / requirements.txt parent
        candidate = d
        for _ in range(4):
            if any(
...
def scan_missing_requirements(root: str = ".") -> List[Dict]:
    """
    Flag Python project dirs that have imports of third-party packages
    but no requirements.txt / pyproject.toml dependency declaration.
    """
    tasks = []
    for proj in _python_project_dirs(root):
        ...
        if has_req or has_pyproject:
            continue
        # Heuristic: look for common third-party imports in .py files under proj
        imports_found = set()
        for path in _walk_files(proj, (".py",)):
            try:
                ...
                for m in re.finditer(
                    r"^(?:from|import)\s+([a-zA-Z_][\w]*)", src, re.MULTILINE
                ):
                    name = m.group(1)
                    if name in PYTHON_CORE_PINS or name.lower() in {
                        k.lower() for k in PYTHON_CORE_PINS
                    }:
                        imports_found.add(name)
            except Exception:
                pass
        if not imports_found:
            # Still flag if there are multiple .py files (likely a package)
        ...
        for imp in sorted(imports_found):
            key = next(
                (k for k in PYTHON_CORE_PINS if k.lower() == imp.lower()), None
            )
            if key:
                ...
                pins.append(imp)
        if not pins:
            pins = ["# Add project dependencies here", "requests>=2.31.0"]
        tasks.append({
            ...
            "body": (
                ...
                + "\n\nCI jobs that do `pip install -r requirements.txt` will fail without this."
            ),
...
def create_requirements_txt(project_dir: str, pins: Optional[List[str]] = None) -> Dict:
    ...
    try:
        ...
        lines = pins or ["# Auto-generated by autonomous-github-agent", "requests>=2.31.0"]
        content = "\n".join(lines) + "\n"
        with open(path, "w") as fh:
            fh.write(content)
        result = {"success": True, "output": f"wrote {path}"}
...
def scan_gha_deprecations(root: str = ".") -> List[Dict]:
    """Detect uses: actions/*@vN that are behind known-good majors."""
    tasks = []
    for path in _walk_files(root, (".yml", ".yaml")):
        if "/.github/workflows/" not in path.replace("\\", "/"):
            # also allow nested agent workflows
            ...
            if cur_major is not None and good_major is not None and cur_major < good_major:
                tasks.append({
                    ...
                    "body": (
                        ...
                        f"Recommended: `{good}` (known-good for this monorepo).\n\n"
                        f"Stale action majors cause deprecation warnings and eventual breakage."
                    ),
                    "path": path,
                    "action": action,
                    "old_version": version,
...
def fix_gha_deprecation(path: str, action: str, old_version: str, new_version: str) -> Dict:
    """Replace uses: action@old with uses: action@new."""
    result = {"success": False, "output": "", "error": ""}
    try:
        with open(path, "r", errors="ignore") as fh:
            src = fh.read()
        pattern = re.compile(
            rf"(uses:\s*{re.escape(action)}@){re.escape(old_version)}"
        )
...
def scan_stale_cache_paths(root: str = ".") -> List[Dict]:
    """Detect cache-dependency-path entries that point at missing files."""
    tasks = []
    for path in _walk_files(root, (".yml", ".yaml")):
        if "workflows" not in path.replace("\\", "/"):
            continue
        try:
            ...
            continue
        # Simple line-based detection of cache-dependency-path values
        for m in re.finditer(
            r"cache-dependency-path:\s*[|>]?\s*\n?((?:\s+[-\w./]+\n?)+)",
            src,
        ):
            block = m.group(1)
                ...
                if not os.path.exists(candidate):
                    tasks.append({
                        ...
                        "body": (
                            ...
                            f"This produces 'paths not resolved' warnings and breaks caching."
                        ),
                        "path": path,
                        "missing_path": line,
                        "impact": 2.3,
...
def collect_all_tasks(root: str = ".") -> List[Dict]:
    """Run every scanner and return combined list."""
    tasks: List[Dict] = []
...
def write_agent_report(tasks: List[Dict], solved: int, path: str = "agent-report.json") -> str:
    """Emit structured JSON report for observability."""
    report = {
        "version": "4.5",
